...
const AP_Param::GroupInfo AP_GPS::var_info[] = {
    ...
    // @Bitmask: 0:Use UART2 for moving baseline on ublox,1:Use base station for GPS yaw on SBF,2:Use baudrate 115200,3:Use dedicated CAN port b/w GPSes for moving baseline,4:Use ellipsoid height instead of AMSL for uBlox driver,8:Disable RTCM3 forwarding to MovingBaseline-Rover
    // @User: Advanced
...
#if GPS_MAX_RECEIVERS > 1
    if (instance < GPS_MAX_RECEIVERS) {
        if (((_type[instance] == GPS_TYPE_UBLOX_RTK_BASE) || (_type[instance] == GPS_TYPE_UAVCAN_RTK_BASE)) &&
