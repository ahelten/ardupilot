...
# ifdef INCLUDE_HIGH_PRECISION_GPS
        if (get_type() == AP_GPS::GPS_Type::GPS_TYPE_SBF_DUAL_ANTENNA) {
            ...
            check_new_itow(temp.TOW, sbf_msg.length);
            const AuxAntPositionsSub &ant1 = temp.auxAntPositionArray[0];
            if (temp.N > 0 && ant1.Error == 0 && ant1.AmbiguityType == 0) {
                // valid RTK integer fix
                const float rel_heading_deg = degrees(atan2f(ant1.DeltaEast, ant1.DeltaNorth));
