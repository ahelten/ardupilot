...
AP_GPS_UBLOX::_parse_gps(void)
{
    ...
    case MSG_HPPOSLLH:
        ...
        if (option_set(AP_GPS::HeightEllipsoid)) {
            state.location.alt    = _buffer.hpposllh.altitude_ellipsoid / 10;
        } else {
            state.location.alt    = _buffer.hpposllh.altitude_msl / 10;
        }
        state.have_undulation = true;
        state.undulation = (_buffer.hpposllh.altitude_msl - _buffer.hpposllh.altitude_ellipsoid) * 0.001;
...
{
#if GPS_MOVING_BASELINE
    if (_hardware_generation == UBLOX_F9 || _hardware_generation == UBLOX_M10) {
        return true;
    }
