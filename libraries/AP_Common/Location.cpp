...
bool Location::get_vector_xy_from_origin_NE(T &vec_ne) const
{
    ...
    Vector2f vec_ne_f = get_distance_NE(ekf_origin);
    vec_ne.x = vec_ne_f.x * -100; // invert and convert to cm
    vec_ne.y = vec_ne_f.y * -100; // invert and convert to cm
#else
...
{
#ifdef INCLUDE_HIGH_PRECISION_GPS
    const double dlat = ofs_ned.x * LOCATION_SCALING_FACTOR_INV;
    const double dlng = (ofs_ned.y * LOCATION_SCALING_FACTOR_INV) / longitude_scale(lat+dlat/2);
    int64_t hplat = ((int64_t)lat * 100) + lat_hp;
    int64_t hplng = ((int64_t)lng * 100) + lng_hp;
...
// -- Here is the comment in original branch that resized this to 20:
//JO:
...
void Location::get_latlng(Location &location) const
{
    location.lat = lat;
    location.lng = lng;
